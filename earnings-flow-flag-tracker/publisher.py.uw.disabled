"""
AZTMM Earnings Flow Flag — publisher
...
Responsibilities:
  ...
  2. Run the brand-policy scrubber — block publish on any forbidden phrase
     in user-visible output. Special-case the earnings-flow forbidden
     terms: "earnings play", "trade the earnings".
  3. Append the day's headline metric to history.json (sparkline).
  4. Write dual-output JSON (public + internal) + the rendered HTML.
"""
...
logger = logging.getLogger("earnings_flow.publisher")


# ---------------------------------------------------------------------------
# Brand policy — forbidden phrases (case-insensitive substring + regex)
# Inherited from FRAMEWORK-RECAP-v2 + earnings-specific bans.
# ---------------------------------------------------------------------------
...
FORBIDDEN_PHRASES_BASE = [
    ...
    " uw ",
    "★",  # star char
    # Earnings-specific bans (per build spec):
    "earnings play",
    "earnings-play",
    "trade the earnings",
...
# Allow-list — substrings whose advisory words are legitimately present
# (standard disclaimers, etc.). The vendor/secret-sauce scan runs against
# the FULL HTML, so it still catches anything.
ADVISORY_ALLOWLIST = [
    ...
    "options around earnings carry elevated risk",
    "elevated risk",
    # Permitted call-related terminology (noun phrases describing past
    # observation, not present-tense recommendations).
    "call-tilted",
    "heavily call-tilted",
    "all call-side",
    "bullish call-tilted",  # never appears but defensive
    "call side",
    "call-side",
]
...
def write_outputs(agg: dict, html: str, out_dir: Path, date: str) -> dict:
    ...
    public_path = out_dir / f"earnings-flow-{date}.public.json"
    internal_path = out_dir / f"earnings-flow-{date}.internal.json"
    html_path = out_dir / f"earnings-flow-{date}.html"
