"""
AZTMM Earnings Flow Flag — orchestrator

Runs end-to-end:
  fetch -> aggregate -> update history -> render -> brand-check -> dual-output write
...
logger = logging.getLogger("earnings_flow.run")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run AZTMM Earnings Flow Flag")
    ...
    # Idempotency guard
    pub_path = out_dir / f"earnings-flow-{target_date}.public.json"
    ...
    try:
        ...
        log["steps"].append({
            "step": "fetch", "ok": True,
            "forward_window": bundle.get("forward_window"),
            "earnings_universe_size": len(bundle.get("universe_tickers") or []),
            "tickers_probed": bundle["data_quality"]["tickers_probed"],
    ...
    try:
        ...
        log["steps"].append({
            ...
            "flagged_count": agg["internal"]["flagged_count"],
        })
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        history_data = update_history(
            HISTORY_PATH,
            tracker_slug="earnings-flow-flag-tracker",
            label=headline.get("label", "Flagged upcoming reporters"),
            date=target_date,
        ...
        sparkline_ctx = {
            ...
            "headline_metric_label": "Flagged upcoming reporters",
    ...
    if not check["ok"]:
        ...
        nr_path = DATA_DIR / "needs-review" / f"earnings-flow-{target_date}-NEEDS-REVIEW.html"
        nr_path.write_text(
    ...
    sys.stdout.write(json.dumps({
        ...
        "rows_public": len(agg["public"]["rows"]),
        "flagged_count": agg["internal"]["flagged_count"],
        "earnings_universe_size": agg["internal"]["earnings_universe_size"],
