"""
AZTMM Squeeze Watch — orchestrator

Runs end-to-end:
  fetch -> aggregate -> render -> brand-check -> write dual-output

CLI:
  --date YYYY-MM-DD   target date (default: today UTC)
  --dry-run           do everything except write final outputs to sample-output/
  --force-publish     reserved for the GH workflow wrapper

Idempotent: re-running for the same date overwrites the dated artifacts.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run AZTMM Squeeze Watch")
    ...
    # 0a. Idempotency guard: skip if dated artefact already exists.
    pub_path = out_dir / f"squeeze-{target_date}.public.json"
    ...
    try:
        ...
        log["steps"].append({
            "step": "fetch", "ok": True,
            "tickers_kept": len(bundle["tickers"]),
            "tickers_filtered": len(bundle["data_quality"]["tickers_filtered"]),
    ...
    # 4. Brand check — must be CLEAN to proceed
    ...
    if not check["ok"]:
        ...
        # Write the failing HTML next to logs for triage
        (DATA_DIR / "needs-review").mkdir(parents=True, exist_ok=True)
        nr_path = DATA_DIR / "needs-review" / f"squeeze-{target_date}-NEEDS-REVIEW.html"
        nr_path.write_text(
            f"<!-- BRAND-POLICY BLOCK: hits = {check['hits']!r} -->\n" + html
        )
        return 5

    # 5. Write dual outputs (idempotent — overwrites on same date)
